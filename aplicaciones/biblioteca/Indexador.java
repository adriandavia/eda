...
public class Indexador {
            ...
            while (libro.hasNext()) {
                ...
                for (int i = 0; i < words.length; i++) {
                    ...
                    if (esTermino(clave)) {
                        ...
                        ListaConPI<Indice> valores = null;
                        if (indices.recuperar(clave) != null) {
                            valores = indices.recuperar(clave);
                        } else {
                            valores = new LEGListaConPI<Indice>();  
                        }
                        valores.insertar(ind);
                        indices.insertar(clave,valores);
                        t2 = System.nanoTime();
                    }
                    tmp += (t2 - t1);
                }
    ...
    public ListaConPI<String> indiceDe(String pal) {
        ...
        ListaConPI<Indice> claves = indices.recuperar(pal);
        for(claves.inicio(); !claves.esFin(); claves.siguiente()) {
            res.insertar(claves.toString());
        }
        return res;   
    }
