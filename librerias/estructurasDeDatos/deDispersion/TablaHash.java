...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    public ListaConPI<C> claves() {
        ...
        for(int i = 0; i < elArray.length; i++) {
            ...
            for(l.inicio(); !l.esFin(); l.siguiente()){
                EntradaHash<C, V> eH = l.recuperar();
                C c = eH.clave;
                res.insertar(c);
            }
        }
    ...
    protected final void rehashing() {
        //COMPLETAR
        ListaConPI<EntradaHash<C, V>>[] old = elArray;
        
        int capacidad = siguientePrimo((int) old.length * 2);//(talla / FACTOR_CARGA));
        elArray = new LEGListaConPI[capacidad];
        for (int i = 0; i < elArray.length; i++) { 
            elArray[i] = new LEGListaConPI<EntradaHash<C, V>>();
        }
        ...
        for(int i = 0; i < old.length; i++) {
            ...
            for(l.inicio(); !l.esFin(); l.siguiente()){
                EntradaHash<C, V> eH = l.recuperar();
                int pos = indiceHash(eH.clave);
                elArray[pos].insertar(eH);
            }
        }
    } 
    ...
    public final double desviacionTipica() {
        // raizcuadrada(sumatorio(l1 - l2)2)/N)
        int longCubeta = 0; //li
        double promedio = 0.0;
        double sumatorio = 0.0;
        //calculamos el promedioCubetas
        for(int i = 0; i < elArray.length; i++){
            promedio += elArray[i].talla();
        }
        double promedioCubetas = promedio / (double) elArray.length;
        
        for(int i = 0; i < elArray.length; i++){
            sumatorio += Math.pow(elArray[i].talla() - promedioCubetas, 2);
        }
        
        
        return Math.sqrt(sumatorio / (double) elArray.length);
    }
