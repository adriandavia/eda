...
public class ABB<E extends Comparable<E>> {
    ...
    protected NodoABB<E> construirEquilibrado(E[] v, int ini, int fin) {
        // COMPLETAR 
        NodoABB<E> izquierda, derecha, raiz;
        if(ini > fin) { return null; }
        else {
            int mitad = (fin+ini)/2;
            izquierda = construirEquilibrado(v,ini,mitad-1);
            derecha = construirEquilibrado(v,mitad+1,fin);
            raiz = new NodoABB<E>(v[mitad], izquierda, derecha);
            return raiz;
        }
    }
